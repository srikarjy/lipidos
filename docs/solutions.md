...
### 2026-08-14 — Re-measured search latency; old "3.1 ms" figure was matmul-only, not end-to-end

**Question:** `PROGRESS.md`'s Scale table had "search latency: exact brute-force cosine, sub-5ms at this scale" with no benchmark script behind it and no record of what exactly was timed. Needed a real, repeatable number at the current corpus size (281 papers / 12,563 chunks, post PDF-ingestion + Colab re-embed).

**What was actually being called "search latency" before, reconstructed:** just `vecs @ qvec` + `argsort` — the pure cosine matmul, timed in isolation, not the `search_chunks()` function real queries actually call. Confirmed by re-measuring that isolated step today: 1.8 ms p50 at the current (smaller, 12,563-chunk) corpus — even faster than the old 3.1 ms figure, consistent with "matmul only" since this corpus is smaller than the "48k chunks" the old number cited.

**What `search_chunks()` actually costs end-to-end:** the same matmul, then one SQLite row-fetch per pooled candidate (`pool=max(k*10, 50)` → 50 individual `SELECT`s joining `chunks`/`papers`) plus the per-paper cap/merge logic (`query.py`'s diversity-aware retrieval, added 2026-07-23). Wrote `scripts/bench_latency.py`: embeds all 35 real `eval_queries.py` questions once (outside the timed loop — query embedding is a one-time BGE forward pass, not part of the "does brute-force search stay fast" claim), then times `search_chunks()` itself over 20 reps × 35 queries = 700 runs, k=5. Result: **83.7 ms p50 / 135.4 ms mean / 348.2 ms p95**.

**Decision:** report the full `search_chunks()` number as the headline in `PROGRESS.md`, not the matmul-only one — it's what a real query actually pays, and the old figure's undocumented scope (no benchmark script, no note that DB I/O was excluded) is exactly the kind of unlabeled number this project's "measured, not estimated" standard exists to prevent. Kept both numbers in `PROGRESS.md` with the breakdown explained, rather than silently swapping one unlabeled number for another.

---

### 2026-08-14 — Re-measured reference DOI resolution; clarified what "resolved" has always meant

**Question:** `PROGRESS.md` flagged "references resolved | not re-measured since v3 (was 19,674)" as stale. Re-measure against the current corpus (281 papers, post PDF-ingestion).

**What "resolved" actually means, confirmed by reading `parse_jats.py`:** a reference row's `doi` column is populated only when the source JATS XML's `<element-citation>` carries a `<pub-id pub-id-type="doi">` element (`parse_jats.py:243-247`) — there is no Crossref/DOI-lookup step anywhere in the codebase that resolves a *missing* DOI for a reference (confirmed: `grep -rl crossref.org scripts/*.py` only matches `backfill_titles.py`, which resolves *paper titles*, not reference DOIs). So "resolved" has always meant "DOI present in the source XML," not independently verified — the old "19,674" figure almost certainly meant the same thing, just uncredited as to method. Recording the method explicitly this time so the next re-measurement doesn't have to reconstruct it.

**Also confirmed:** the 61 PDF-sourced papers (`parse_pdf.py`) have zero rows in `references` — the PDF parser stops at the References section heading (`STOP` regex) rather than extracting citations from it, so this metric only ever applies to the 219 papers (of 281) that have any parsed reference list, all JATS-sourced.

**Result:** 11,307 of 12,774 references (88.5%) carry a DOI, across 219 papers. Query: `SELECT COUNT(*) FROM "references" WHERE doi IS NOT NULL AND TRIM(doi) != ''` over `SELECT COUNT(*) FROM "references"`.

---

### 2026-08-14 — Fine-tuning corpus kept fully separate from the retrieval corpus

**Question:** need 100K+ abstracts to QLoRA fine-tune Phi-3.5 Mini for domain fluency. Where does this data live, and how does it relate to `data/papers.db`?

**Decision: separate directory (`data/finetune_corpus/`), separate format (raw JSONL abstracts, no parsing), never merged into `papers.db`.** The retrieval corpus's entire value is "every claim resolves to a DOI" — 281 hand-selected, full-text-parsed, citation-mapped papers (see the 2026-07-15 "Selection, not scale" entry: broadening that corpus by keyword alone produces 36,822 noisy hits vs 4,316 clean ones). A fine-tuning corpus needs the opposite shape: broad, high-volume, abstract-only, no full-text/citation parsing, and it's fed to the model as raw training signal rather than retrieved-and-cited evidence. Storing them in the same DB or directory would (a) make it easy to accidentally retrieve from the noisy pool, undermining the DOI guarantee, and (b) blur which corpus a given number in `PROGRESS.md` refers to. Kept as flat JSONL specifically so nothing downstream can mistake it for retrievable, grounded evidence.

**Corpus construction (`scripts/fetch_finetune_corpus.py`):** two PubMed E-utilities pools, MeSH-based (not full-text, not PMC-restricted — this only needs abstracts, not tables/citations):
- **Pool A** (bulk lipid biochemistry/lipidomics/membrane biophysics): `("lipids"[MeSH] OR "lipidomics"[MeSH] OR "lipid bilayers"[MeSH] OR "membrane fluidity"[MeSH] OR "unsaturated fatty acids"[MeSH] OR "phospholipids"[MeSH]) AND hasabstract[text] AND English[Language]` — 1,092,098 total hits in PubMed; pulled 92,384.
- **Pool B** (spectroscopic method vocabulary, deliberately not lipid-restricted — Raman and IR share peak-assignment/selection-rule language constantly discussed together in this literature): `("spectrum analysis, Raman"[MeSH] OR "spectroscopy, fourier transform infrared"[MeSH] OR "vibrational spectroscopy"[tiab]) AND hasabstract[text] AND English[Language]` — 88,434 total hits; pulled 16,032, capped to 15,281 unique (751 already covered by Pool A) to stay under a 20% max-share cap on the combined corpus.

**NCBI paging obstacle:** esearch hard-caps `retstart + retmax` at 9,999 records per query (confirmed live — both pools' single-query hit counts are >88K, far past that). Standard `usehistory=y` + `retstart` paging still hits the same 9,999 ceiling for PubMed specifically (NCBI's own error message points at EDirect as the intended workaround). Sidestepped by paging per calendar month instead (`month_windows()`), walking backwards from the current month — each pool's monthly hit count is comfortably under 9,999 (~3-4K/month for Pool A), so no bucket ever needs further splitting. Walks backward only as far as needed to hit each pool's target count (Pool A: ~2 years back; Pool B: ~1.5 years), not the full multi-decade history — domain-adaptation fluency doesn't need exhaustive historical coverage, and it would cost a lot of unnecessary NCBI traffic under the unkeyed 3 req/sec limit.

**Result:** 107,665 unique abstracts (Pool A 92,384 / 85.8%, Pool B unique 15,281 / 14.2% — within the planned 80/20-85/15 mix), all with real abstract text (`hasabstract[text]` held, 0 empty). Stored in `data/finetune_corpus/abstracts.jsonl` (196 MB, gitignored via the blanket `data/` rule), each record tagged `pool: "A"|"B"`. `data/finetune_corpus/manifest.json` records the exact queries and counts, same pattern as `fetch_corpus.py`'s `data/manifest.json` for the retrieval corpus.

---

### 2026-08-15 — QLoRA fine-tune: platform switch, GPU-capacity preemption, and a wrong assumption about bucket-mount persistence

**Objective, decided explicitly (not left implicit):** next-token/causal-LM domain adaptation on raw `title + abstract` text from the fine-tuning corpus above, not an instruction format. There's no specific downstream task shape here — the goal is domain fluency (lipid/Raman vocabulary), which the model then uses inside the existing grounded-retrieval + citation-checking pipeline. `scripts/finetune_phi3.py` already covers the separate instruction-tuned (peaks + evidence → cited interpretation) case on the small retrieval corpus; this is a deliberately different objective on the much larger corpus.

**Platform history:** started on Hugging Face Jobs (A10G, plain `transformers` + `bitsandbytes`). Several dependency-version crashes before it trained at all: `trust_remote_code=True` pulled Phi-3.5's own `modeling_phi3.py`, which calls a `DynamicCache` API removed from current `transformers` — fixed by dropping `trust_remote_code` and using `transformers`' native Phi-3 support (present since 4.41). Then an open-ended `transformers>=4.44` pin resolved to a version that had renamed `TrainingArguments`'s `warmup_ratio` — fixed by pinning known-stable versions. Then the pinned `bitsandbytes==0.44.1` turned out to be too old for the container's CUDA/triton stack (`ModuleNotFoundError: triton.ops`) — fixed by unpinning just that one package.

Once training actually started, the default settings (batch 4, grad_accum 4, gradient checkpointing on, max_length 1024) measured at ~19 hours per epoch — far past any reasonable job budget. Fixed with a bigger batch, checkpointing off, and `max_length=512` (measured: mean abstract 239 words/~310 tokens, p95 372 words/~480 tokens, so 512 loses almost nothing).

**Switch to Kaggle + Unsloth, then back to HF Jobs:** asked to move to Kaggle Notebooks + Unsloth for a zero-cost, full-corpus run. Got Kaggle API auth working (the account's real username, `srikarjy`, only surfaced by pushing a test kernel and reading the resulting URL — the locally configured `kaggle.json` had a stale, wrong username). But Kaggle's API assigns whichever GPU is the account's current default with no field to request a specific accelerator, and it defaulted to a P100 (Pascal, `sm_60`) — which Unsloth does not support (needs Turing/`sm_75`+), confirmed live (`Tesla P100 ... is not compatible with the current PyTorch installation`). No API path to force T4x2 without manual browser-UI intervention of uncertain persistence. Reconsidered and went back to HF Jobs, using **Unsloth on the A10G** instead — gets Unsloth's speed without the Kaggle accelerator-selection problem.

**Unsloth-specific fixes:** `unsloth` must be imported before `transformers`/`peft` or it silently loses optimizations (`Unsloth should be imported before transformers, peft`) — reordered imports. The pinned `transformers==4.46.3` from the earlier plain-transformers debugging was too *old* for this Unsloth release (`ModuleNotFoundError: transformers.models.qwen3`) — fixed by unpinning transformers/peft/accelerate/bitsandbytes entirely and letting Unsloth's own dependency resolution pick compatible versions. `FastLanguageModel.for_training(model)` crashed (`AttributeError: 'PeftModelForCausalLM' object has no attribute '_flag_for_generation'`) because it's only meant to undo a prior `for_inference()` call on the *same* object, and `for_inference` had been called on `base_model` before `get_peft_model` wrapped it into a new object — removed the unnecessary call. At `batch_size=16` with checkpointing off, hit a live CUDA OOM at step 0 (22.28 GiB used of the A10G's 22.3 GiB) — fixed by dropping to `batch_size=8`/`grad_accum=2` and switching to Unsloth's own optimized checkpointing (`use_gradient_checkpointing="unsloth"`, not plain `False` or HF's default, since Unsloth's claim is near-zero speed cost for full memory savings).

**The real cost: GPU-capacity preemption, and a wrong persistence assumption.** Once training ran cleanly, jobs kept dying with `exit code 143` (SIGTERM, no Python traceback) after anywhere from 25 minutes to ~3 hours — a `hf jobs run` test job separately observed stuck in `SCHEDULING: "Waiting for requested hardware to become available"`, pointing at GPU-capacity contention on the A10G pool, not an application bug or (confirmed by the user) exhausted billing credit. The first fix attempt trusted that `/data` (the bucket-mounted `output_dir` `hf jobs uv run` provides) would persist Trainer's own checkpoints across separate job invocations. **This was wrong, confirmed live**: a checkpoint written by one job was invisible to a completely separate job trying to read the same `/data` path back — each job's bucket mount is evidently scoped per-invocation, not a shared persistent volume. This cost one fully-wasted ~2.5-hour run (v8) with zero recoverable progress.

**Fix: checkpoint persistence via a Hub repo, not the bucket mount.** Added a private model repo (`srikarjy025/lipidos-domain-adapt-checkpoints`) and a `TrainerCallback.on_save` hook that pushes each local checkpoint to a fixed `latest/` path in that repo (fixed path, not per-step, since Trainer checkpoint filenames — `trainer_state.json`, `adapter_model.safetensors`, etc. — are the same regardless of step number, so each push cleanly overwrites rather than accumulating). At startup, the script tries `snapshot_download` on that repo and resumes from it if a valid checkpoint exists. **Verified with a cheap two-job smoke test before spending any GPU time on it** (one `cpu-basic` job uploads a fake checkpoint, a separate `cpu-basic` job downloads and confirms it) — this is the check that would have caught the `/data` assumption being wrong for a few cents instead of a wasted GPU-hour, and is the right pattern going forward for any cross-job-persistence assumption on this platform.

That smoke test's dummy checkpoint (just a `trainer_state.json`, no real weights) then caused its own bug: the real run found it, tried to resume, and crashed with `ValueError: Can't find a valid checkpoint` after loading the model but before training — because the resume check only verified `trainer_state.json` existed, not that real adapter weights were present too. Fixed by requiring both `trainer_state.json` and an `adapter_model.safetensors`/`.bin` file before treating a downloaded checkpoint as resumable, falling back to a fresh start otherwise (fail closed, not fail crashing). Also cleared the stale test checkpoint from the repo.

**Checkpoint granularity:** dropped `save_steps` from 1000 to 250 once it was clear preemption could hit within an hour — caps the cost of any single preemption at ~250 steps of lost work (~20 minutes) instead of up to 1000.

**Final run:** capped at `max_steps=4000` (60.6% of one epoch over the 105,665-example corpus, i.e. 64,000 examples) to fit a fixed compute budget rather than target a full epoch — domain-adaptation fine-tunes on a corpus this size don't need multiple passes or even a full single pass to show a real fluency signal; the acceptance bar was "measurably lower perplexity than base," not "trained on literally every example." Needed 4 separate job launches (v9 crashed immediately on the stale test checkpoint; v10 ran to step ~1426 before preemption; v11 resumed from 1250 and ran to step ~3400 before another preemption; v12 resumed from 3250 and completed) — each relaunch identical to the last except for the Hub-checkpoint resume actually working from v10 onward. **Result: base perplexity 4.911 → fine-tuned 3.955, a 19.5% reduction**, measured on the same 2,000-example held-out set for both. Adapter pushed to `srikarjy025/lipidos-phi3-domain-adapt` (private). This model still only answers through the existing grounded-retrieval + citation-checking pipeline (`answer.py`) — fine-tuning changed domain fluency, not the citation-verification requirement; a fine-tuned model generating an ungrounded claim is still subject to the same citation check as before, unchanged by this work.

---

### 2026-08-15 — Verifying citation-checking survives the fine-tune: a format mismatch, and a real check

**Question:** the fine-tune's acceptance criterion explicitly required verifying "the existing citation-checking logic still rejects any generated sentence that cites evidence not actually in the retrieved set, verified with the fine-tuned model in place of the base one." Is that actually true?

**First obstacle: the check as written can't run against the fine-tuned model at all.** `answer.py`'s citation check operates on generations from `mlx-community/Phi-3.5-mini-instruct-4bit` — an MLX-quantized weight format, loaded via `mlx-lm`. The fine-tune produced a PEFT/`safetensors` LoRA adapter trained via Unsloth/bitsandbytes for CUDA — a different weight format entirely, not loadable by `mlx-lm`. There is no existing path to literally "swap the fine-tuned model into `answer.py`" without a format conversion. Solved by porting the check itself instead of the model: `scripts/verify_finetuned_citations.py` reuses `answer.py`'s exact `SYSTEM_PROMPT`, evidence-block format, and `check_citations` regex, but loads the fine-tuned model via `transformers`/`peft` (the adapter's native format) rather than `mlx_lm`.

**Test design:** real retrieval, not synthetic evidence — ran the actual `search_chunks` pipeline locally (same retrieval `answer.py` uses) for 2 real domain questions and 1 deliberate out-of-domain probe ("how do I bake sourdough bread at home", same OOD-floor question used in `eval_queries.py`), uploaded the resulting evidence blocks to a small private dataset so a remote GPU job could generate against them without needing `papers.db`/`chunk_vectors.npy` to leave the machine.

**Result: 0/3 hallucinated evidence IDs.** Both domain questions cited only real evidence numbers (1-5) with substantive, on-topic answers; the OOD question correctly answered "the provided evidence does not contain information on how to bake sourdough bread... therefore, it cannot answer the question" rather than inventing a connection. The citation check's actual guarantee — no fabricated evidence *numbers* — holds identically with the fine-tuned model.

**What this does NOT verify, stated explicitly:** `check_citations` is a narrow, syntactic check (does the cited number exist in range 1-n), not a semantic one (does the cited evidence's *content* actually support the claim attached to it). Spot-checking the PE-vs-PC test case, the model's claim that a specific band is "associated with phosphatidylethanolamine... linked to the oleoyl chain" is a real but subtler overreach from what evidence #3 literally states — a genuine soft-hallucination the regex can't catch. This is a pre-existing scope limit of the citation check itself (true before this fine-tune, on the base model, exactly as much as after) — not a regression introduced by fine-tuning, and not something this verification claims to have ruled out.

---

### 2026-08-15 — Wiring Context Builder + the fine-tuned model together: two pieces that had never been connected

**Question:** re-reading `lipid-raman-rag-blueprint.md` end to end (at the user's request, before committing tonight's work) to find the actual next step, rather than assuming "more fine-tuning" was it.

**What was actually already there, checked against the blueprint's own phase list:** `scripts/context_builder.py` (Phase 4) and `scripts/raman_integration.py` (Phase 5) both already exist, both correctly labelled in their own docstrings, both functionally complete for what they claim to do. But `grep -rn "context_builder\|raman_integration" scripts/answer.py` returns nothing — `answer.py` (Phase 2) still builds its own simpler, prose-only evidence block and has never called Context Builder. Context Builder and Raman Integration both *build* evidence and print it, but neither one *generates* an interpretation from it — there was no code path from "merged, cited, three-track evidence" to "an LLM's answer over that evidence," fine-tuned or otherwise. README's Status table had these marked "Not started," which was simply stale, not a reflection of missing code.

**A second, more consequential finding:** the blueprint's actual intended Phase 6 artifact — a Gemma/Phi model fine-tuned on (peaks + retrieved evidence + database facts) → expert interpretation pairs, i.e. a *reasoning* fine-tune, not a fluency one — is `data/finetuned_phi3_mlx/`. That directory contains only `adapter_config.json` (an MLX-LoRA training config) with **no `adapters.safetensors` or any other weight file**. The training run that config belongs to never reached a save point, or its output was never committed. This means the blueprint's own Phase 6 was never actually completed, and tonight's QLoRA domain-adaptation adapter (`srikarjy025/lipidos-phi3-domain-adapt`) — trained for a separate, resume-claim-driven reason, not literally "Phase 6" — is currently the *only* complete, working fine-tuned artifact anywhere in this project.

**Fix, scoped to what's actually missing:** connect Context Builder's evidence to the fine-tuned model, with the same citation-checking bar as `answer.py`. Split into two scripts rather than one, for a concrete hardware reason: Context Builder needs `papers.db`/`chunk_vectors.npy` (local, fast, BGE runs fine on this machine's MPS) but the fine-tuned model is ~7.6 GB in bf16, too tight for this project's 8 GB M2 development machine (`answer.py`'s base-model path avoids this by using an MLX 4-bit quantization instead, but the QLoRA adapter is PEFT/`safetensors` format, not MLX-loadable). `scripts/build_evidence.py` runs locally, builds evidence via `context_builder.build_context_block()`, uploads it as JSON to a small private dataset (`srikarjy025/lipidos-pipeline-evidence`). `scripts/generate_finetuned_answer.py` runs on a remote GPU, downloads that evidence, generates with the fine-tuned model using `answer.py`'s exact `SYSTEM_PROMPT`, and runs the same `check_citations` regex -- same pattern already proven working by `verify_finetuned_citations.py` earlier the same night.

**Verified end-to-end, real question, no synthetic data:** "how is the 1655/1440 ratio used to quantify lipid unsaturation?" → Context Builder returned 5 merged evidence items → fine-tuned model generated an answer citing evidence #2, #3, and #5 → zero hallucinated citation IDs. First time all three pieces (merged multi-track evidence, the fine-tuned model, citation checking) have run together as one connected pipeline in this project.
